...
describe('Advanced Textarea', () => {
    ...
    it('should change the counter message correctly', async () => {
        ...
        expect(textareaCounterElement.nativeElement.textContent).toBe(' 3 characters over the limit ');
    });
    ...
    it('should change the counter message correctly when within limit', async () => {
        ...
        const textareaCounterElement = fixture.debugElement.query(By.css('.fd-textarea-counter'));
        expect(textareaCounterElement.nativeElement.textContent).toBe(' 2 characters remaining ');
    });

    // clipboard actions have stricter security and only work on secure https urls.
    // https://developers.google.com/web/updates/2018/03/clipboardapi
    // Therefore, ignoring this test.
    xit('should handle paste interaction', async () => {
        ...
        textareaElement.textareaElement.nativeElement.focus();
        await wait(fixture);
        textareaElement.textareaElement.nativeElement.dispatchEvent(pasteEvent);
        await wait(fixture);

        expect(host.form.get('basicTextarea').value).toBe('textnew value');
    });
